...
def load_goodtoon(path):
    ...
    source = meta["sources"][0]
    expected_source = {"id": 760550510744678728, "name": "Goodtoon 웹툰", "lang": "ko", "baseUrl": "https://www.goodtoon004.com"}
    if any(source.get(key) != value for key, value in expected_source.items()):
...
def load_blacktoon(path):
    meta = json.loads(path.read_text(encoding="utf-8"))
    expected = {
        "packageName": BLACKTOON,
        "versionCode": BLACKTOON_CODE,
        "versionName": BLACKTOON_VERSION,
        "extensionLib": "1.4",
        "contentWarning": 2,
        "name": "Blacktoon 웹툰",
    }
    if any(meta.get(key) != value for key, value in expected.items()) or len(meta.get("sources", [])) != 1:
        fail("Built Blacktoon metadata is invalid")
    source = meta["sources"][0]
    expected_source = {"id": 7080800841003944426, "name": "Blacktoon 웹툰", "lang": "ko", "baseUrl": "https://blacktoon423.com"}
    if any(source.get(key) != value for key, value in expected_source.items()):
        fail("Built Blacktoon source identity is invalid")
    return meta
...
def main():
    ...
    parser.add_argument("--blacktoon-apk", type=Path, required=True)
    parser.add_argument("--blacktoon-source-info", type=Path, required=True)
    parser.add_argument("--blacktoon-icon", type=Path, required=True)
    ...
    blacktoon_apk = args.blacktoon_apk.resolve()
    blacktoon_icon = args.blacktoon_icon.resolve()
    if any(not asset.is_file() for asset in (toonkor_apk, toonkor_icon, goodtoon_apk, goodtoon_icon, toon11_apk, toon11_icon, blacktoon_apk, blacktoon_icon)):
    ...
    blacktoon = load_blacktoon(args.blacktoon_source_info.resolve())
    ...
    blacktoon_entry = index.extensionList.extensions.add()
    update_entry(blacktoon_entry, blacktoon, args.repository, blacktoon_apk.name, BLACKTOON_ICON)
    ...
    copy(blacktoon_apk, out / "apk" / blacktoon_apk.name)
    copy(blacktoon_icon, out / "icon" / BLACKTOON_ICON)
    ...
    if entries[BLACKTOON].versionCode != BLACKTOON_CODE or entries[BLACKTOON].sources[0].id != 7080800841003944426:
        fail("Reconstructed Blacktoon entry is invalid")
    ...
    readable.append({"name": "Tachiyomi: Goodtoon 웹툰", "pkg": GOODTOON, "apk": goodtoon_apk.name, "lang": "ko", "code": 10, "version": GOODTOON_VERSION, "nsfw": 1, "sources": [{"name": "Goodtoon 웹툰", "lang": "ko", "id": "760550510744678728", "baseUrl": "https://www.goodtoon004.com", "versionId": 1, "hasCloudflare": 0}]})
    ...
    readable.append({"name": "Tachiyomi: Blacktoon 웹툰", "pkg": BLACKTOON, "apk": blacktoon_apk.name, "lang": "ko", "code": 35, "version": BLACKTOON_VERSION, "nsfw": 1, "sources": [{"name": "Blacktoon 웹툰", "lang": "ko", "id": "7080800841003944426", "baseUrl": "https://blacktoon423.com", "versionId": 1, "hasCloudflare": 0}]})
    ...
    print(f"Published Toonkor {toonkor_apk.name}, Goodtoon {goodtoon_apk.name}, 11toon {toon11_apk.name}, and Blacktoon {blacktoon_apk.name}")
